...
Related to the choice of the backend compiler. The "backend compiler" takes as input the code generated by Devito and translates it into a shared object. Supported backend compilers are `gcc`, `icc`, `pgcc`, `clang`. For each of these compilers, Devito uses some preset compilation
flags (e.g., -O3, -march=native, etc). The default backend compiler is `gcc`. To change it, one should set the `DEVITO_ARCH` environment variable to a different value; run
```
from devito import print_defaults
print_defaults()
```
...
#### DEVITO_PLATFORM
Env variable DEVITO_PLATFORM is mostly needed when running on GPUs or ARM architectures. Can be also used to specify Intel architectures like Haswell, Broadwell, SKL and KNL.

#### DEVITO_PROFILING
Choose perfromance profiling level By default set to 'basic'. Set to advanced when benchmarking and doing autotuning. Planned to be integrated with Intel Advisor using 'advisor' (Not supported currently)

#### DEVITO_BACKEND
Choose execution backend. Usually `core`.

#### DEVITO_DEVELOP
Return a suitable MemoryAllocator for the architecture on which the process is running.
For more info, one can have a look here: [def default_allocator():](https://github.com/devitocodes/devito/blob/24ede9131473f69c1e44ba3b852f8654d3fd953e/devito/data/allocators.py#L373)

#### DEVITO_OPT
Choose the performance optimization level. By default set to `advanced`.

#### DEVITO_MPI

Controls MPI in Devito. Use `1`to enable MPI. The most powerful MPI mode is called "full", and is activated setting `DEVITO_MPI=full` instead of `DEVITO_MPI=1`. The "full" mode supports computation/communication overlap.

#### DEVITO_LANGUAGE
Specify generated code language. The default is C code. Use `openmp` for c with OpenMP directives and
`openacc` for C with OpenACC directives.

#### DEVITO_AUTOTUNING

Search towards block sizes that increase performance through improved data locality.
Choose between `off`, `basic`, `aggressive`, `max`.
The autotuning aggressiveness (basic, aggressive, max). A more aggressive autotuning might eventually result in higher runtime performance, but the autotuning phase will take longer.
Suggestion is to use `aggressive` mode when high performance is of high importance. Another choice would be to manually choose block sizes through `-bs` in benchmarking.
...
You can set `DEVITO_DEBUG_COMPILER=1` to emit the command used to compile
the generated code.

#### DEVITO_JIT_BACKDOOR
When set to `1` it allows testing modifications to the generated code.
We suggest looking at [Can I manually modify the c code generated by Devito and test these modifications?](https://github.com/devitocodes/devito/wiki/FAQ#can-i-manually-modify-the-c-code-generated-by-devito-and-test-these-modifications)
